export function getFeaturedVideo() {
  return {
    title: "Understanding GFET Design",
    description: "An in-depth walkthrough of graphene FETs, from theory to layout and simulation.",
    youtubeId: "dQw4w9WgXcQ" // Replace with your actual featured video ID
  };
}

export function getLatestVideos() {
  return [
    {
      title: "GFET Layout Optimization",
      thumbnail: "/thumbnails/layout-opt.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example1",
      duration: "12:34",
      date: "2025-07-20",
    },
    {
      title: "Compact Model Extraction",
      thumbnail: "/thumbnails/compact-model.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example2",
      duration: "9:02",
      date: "2025-07-18",
    },
    {
      title: "AI-assisted Circuit Design",
      thumbnail: "/thumbnails/ai-circuit.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example3",
      duration: "14:58",
      date: "2025-07-15",
    },
    {
      title: "Charge Pump Simulation",
      thumbnail: "/thumbnails/charge-pump.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example4",
      duration: "11:21",
      date: "2025-07-14",
    },
    {
      title: "Intro to Metasurface Design",
      thumbnail: "/thumbnails/metasurface.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example5",
      duration: "13:45",
      date: "2025-07-13",
    },
    {
      title: "Rectenna Matching Networks",
      thumbnail: "/thumbnails/rectenna.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example6",
      duration: "10:50",
      date: "2025-07-11",
    },
    {
      title: "ML for GFET Optimization",
      thumbnail: "/thumbnails/ml-optimization.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example7",
      duration: "15:12",
      date: "2025-07-09",
    },
    {
      title: "DRC and LVS Basics",
      thumbnail: "/thumbnails/drc-lvs.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example8",
      duration: "8:59",
      date: "2025-07-07",
    },
    {
      title: "GFET Layout from Scratch",
      thumbnail: "/thumbnails/layout-from-scratch.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example9",
      duration: "16:04",
      date: "2025-07-05",
    },
    {
      title: "Capacitor Modeling Deep Dive",
      thumbnail: "/thumbnails/capacitor-model.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example10",
      duration: "10:30",
      date: "2025-07-03",
    },
    {
      title: "Energy Scavenging Circuits",
      thumbnail: "/thumbnails/energy-scavenging.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example11",
      duration: "14:11",
      date: "2025-07-01",
    },
    {
      title: "Graphene Bandgap Engineering",
      thumbnail: "/thumbnails/graphene-bandgap.jpg",
      youtubeUrl: "https://www.youtube.com/watch?v=example12",
      duration: "9:47",
      date: "2025-06-28",
    },
  ];
}
